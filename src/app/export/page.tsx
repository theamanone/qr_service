...
const ExportDataPage = () => {
  ...
  const [exportFormat, setExportFormat] = useState<'excel' | 'json' | 'pdf'>(
    'excel'
  )

  let searchParams = new URLSearchParams(document.location.search)
  let exportTypeParam = searchParams.get('format')

  useEffect(() => {
    setExportFormat(
      exportTypeParam === 'pdf'
        ? 'pdf'
        : exportTypeParam === 'json'
        ? 'json'
        : 'excel'
    )
  }, [searchParams])
  ...
  const handleExportClick = async () => {
    ...
    try {
      const response = await fetch(
        `/api/auth/user/data/export?format=${exportFormat}`,
        {
          method: 'GET',
          headers: {
      ...
      >
        ...
        >
          ...
          <div className='flex items-center justify-center mb-4'>
            <label htmlFor='exportFormat' className='mr-4'>
            ...
            <select
              ...
              value={exportFormat}
              onChange={e =>
                setExportFormat(e.target.value as 'excel' | 'json' | 'pdf')
              }
