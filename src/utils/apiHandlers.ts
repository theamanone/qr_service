...
export const createQr = async (formData: any) => {
  try {
    const response = await axiosInstance.post(`${API_BASE_URL}/v1/qr/generate`, formData);
    return response?.data;
  } catch (error: any) {
    handleApiError(error);
  }
...
export const deleteQr = async (qrId: string) => {
  ...
  try {
    ...
    const response = await axiosInstance.delete(`${API_BASE_URL}/v1/qr/delete`, {
...
export const deleteAccount = async () => {
  try {
    const response = await axiosInstance.delete(`${API_BASE_URL}/auth/user/deleteaccount`); // Your API endpoint for deleting the user account
    return response?.data;
  } catch (error) {
...
export const updateUserDetails = async (userDetails: { name: string; email: string }) => {
  try {
    const response = await axiosInstance.patch(`${API_BASE_URL}/auth/user/update`, userDetails); // Your API endpoint
    return response?.data;
  } catch (error) {
